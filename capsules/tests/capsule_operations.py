...
class LocalisedCapsuleLayerTest(unittest.TestCase):
    ...
    def test_transform(self):
        input_tensor = tf.compat.v1.placeholder(tf.float32, shape=[self.M, self.input["vec_dim"], self.input["channels"], self.input["x"],self.input["y"]])
    ...
    def test_localise(self):
        input_tensor = tf.compat.v1.placeholder(tf.float32, shape=[self.M,  self.input["x"],self.input["y"], self.output["vec_dim"], self.input["channels"], self.output["channels"]])
    ...
    def test_route(self):
        ...
        input_tensor = tf.compat.v1.placeholder(tf.float32, shape=input_tensor_shape)
