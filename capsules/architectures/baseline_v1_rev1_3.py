...
class architecture(architecture_base):
    def build(self, input_images):
        ...
        for i in range(1):
            layer1 = tf.layers.conv2d(layer1, 64, 5, padding='same', name='InitConv'+str(i), activation=tf.nn.relu)
        layer2 = layer1
        layer2 = tf.layers.conv2d(layer2, 64, 5, padding='same', name='linear_conv', activation=None)
        # [batch, 128,128, 256]
        ...
        layer3 = tf.layers.conv2d(layer2, 64, 5, padding='same', name='Conv1', activation=tf.nn.relu)
        layer3 = tf.layers.conv2d(layer3, 64, 5, padding='same', name='Conv2', activation=tf.nn.relu)
        layer3 = tf.layers.conv2d(layer3, 32, 5, padding='same', name='Conv3', activation=tf.nn.relu)
        layer3 = tf.layers.conv2d(layer3, 16, 5, padding='same', name='Conv4', activation=tf.nn.relu) # layer 3 ops take up  267680 weights (or at least it should)
        ...
        layer4c = layer4
        for i in range(2):
            layer4a = tf.layers.conv2d(layer4a, 16, 5, strides=(1,1), padding='same', name="capstooutput_tanh"+str(i), activation=tf.nn.tanh)
        for i in range(2):
            layer4b = tf.layers.conv2d(layer4b, 16, 5, strides=(1,1), padding='same', name="capstooutput_relu"+str(i), activation=tf.nn.relu)
        ...
        capslayer = tf.transpose(layer3_keep, [3,1,2,0])
    ...
    def loss_func(self, input_images, ground_truth, validation_input_images, validation_ground_truth):
        ...
        with tf.name_scope('validation'):
            tf.summary.image('input', tf.expand_dims(validation_input_images, axis=3))
            tf.summary.image('ground_truth', tf.expand_dims(validation_ground_truth, axis=3))
            tf.summary.image('output', tf.expand_dims(validation_output, axis=3))
            tf.summary.scalar('psnr', tf.reduce_mean( tf.image.psnr(tf.expand_dims(validation_ground_truth, axis=3), tf.expand_dims(validation_output, axis=3), max_val=1114)))
            tf.summary.scalar('ssim', tf.reduce_mean( tf.image.ssim(tf.expand_dims(validation_ground_truth, axis=3), tf.expand_dims(validation_output, axis=3), max_val=1114)))
