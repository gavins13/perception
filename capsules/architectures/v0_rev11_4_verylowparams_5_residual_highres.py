...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        print(input_images.get_shape().as_list())
        layer1 = init_conv_2d(input_images, 32, "Init", kernel_size=9, type="SAME")# [batch, 1, num_ch, height, width]
        ...
        for i in range(4):
            layer2 = tf.layers.conv2d(layer2, 64, 5, padding='same', name='PrimaryCapsReduce'+str(i), activation=tf.nn.sigmoid)
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 128, 1, padding='same', name='PrimaryCapsReduce.2'+str(i), activation=tf.nn.sigmoid)
        layer2b = layer2
        layer2 = layer2a
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 128, 7, padding='same', name='1PrimaryCaps'+str(i), activation=tf.nn.relu)
        for i in range(4):
            layer2 = tf.layers.conv2d(layer2, 64, 5, padding='same', name='1PrimaryCapsReduce'+str(i), activation=tf.nn.relu)
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 128, 1, padding='same', name='1PrimaryCapsReduce.2'+str(i), activation=tf.nn.sigmoid)
        layer2a = layer2
        layer2 = tf.concat([layer2a, layer2b], axis=3)
        ...
        layer3 = convolutional_capsule_layer_v2(layer2,1,1,'ConvCaps1',output_kernel_vec_dim=16,  num_output_channels=1, strides=[1,1], type="SAME", num_routing=3, use_matrix_bias=False)
        ...
        for i in range(8):
            layer4 =tf.layers.conv2d(layer4, 64, 9, strides=(1,1), padding='same', name="capstooutput"+str(i), activation=tf.nn.relu)
        layer4 = tf.layers.conv2d(layer4, 32, 9, strides=(1,1), padding='same', name="capstooutput."+str(1), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 16, 9, strides=(1,1), padding='same', name="capstooutput."+str(2), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 8, 9, strides=(1,1), padding='same', name="capstooutput."+str(3), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 1, 9, strides=(1,1), padding='same', name="capstooutput."+str(4), activation=tf.nn.tanh)
        output = layer4
        ...
        result = Results(output, capslayer, tf.squeeze(layer4, axis=3) )
        print(">>>>> Graph Built!")
    ...
    def loss_func(self, input_images, ground_truth):
        ...
        with tf.name_scope('accuracy'):
            ...
            capslayer_mean = tf.reduce_mean(capslayer, axis=3, keepdims=True)
            capslayer_res = capslayer - capslayer_mean
            tf.summary.image('capslayer_output_res', capslayer_res, max_outputs=32)

            capsres = tf.slice(res.capsres, [0,0,0], [1,-1,-1])
            tf.summary.image('capsres', capslayer, max_outputs=1)
