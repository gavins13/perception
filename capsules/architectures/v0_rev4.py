...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer2 = depthwise_convolutional_capsule_layer(layer1b, 5,5, "DepthConv1", strides=[1,1], num_output_channels=16, upsampling_factor=None, type="SAME", conv_vector=True)

        layer3 = depthwise_convolutional_capsule_layer(layer2, 9,9, "DepthConv2_red", strides=[2,2], num_output_channels=32, upsampling_factor=None, type="SAME", conv_vector=True)

        layer4 = depthwise_convolutional_capsule_layer(layer3, 5,5, "DepthConv3", strides=[1,1], num_output_channels=32, upsampling_factor=None, type="SAME", conv_vector=True)

        layer5 = depthwise_convolutional_capsule_layer(layer4, 9,9, "DepthConv4_red", strides=[2,2], num_output_channels=32, upsampling_factor=None, type="SAME", conv_vector=True)
        ...
        layer7 = convolutional_capsule_layer_v2(layer6, 3,3,'ConvCaps1',output_kernel_vec_dim=16,  num_output_channels=8, strides=[1,1], type="SAME")
        ...
        layer10 = tf.concat([layer9, layer5], axis=2) #, layer5
        #layer10 = tf.concat([layer9], axis=2) #, layer5


        layer11 = depthwise_convolutional_capsule_layer(layer10, 9,9, "DepthConv7_up", strides=[2,2], num_output_channels=32, upsampling_factor=2., type="SAME", conv_vector=False)

        layer11 = tf.concat([layer11, layer4], axis=2) # layer4 or layer 3


        layer12 = depthwise_convolutional_capsule_layer(layer11, 9,9, "DepthConv8_up", strides=[2,2], num_output_channels=32, upsampling_factor=2., type="SAME", conv_vector=False)

        layer12 = tf.concat([layer12, layer2], axis=2) # layer4 or layer 3

        layer13 = depthwise_convolutional_capsule_layer(layer12, 5,5, "DepthConv9", strides=[1,1], num_output_channels=32, upsampling_factor=1., type="SAME", conv_vector=False)

        layer14 = depthwise_convolutional_capsule_layer(layer13, 5,5, "DepthConv10", strides=[1,1], num_output_channels=16, upsampling_factor=1., type="SAME", conv_vector=False, relu=False)
        layer14 = tf.transpose(layer14, [0,3,4,1,2])
        ...
        layer18 = tf.layers.conv2d(layer16, 128, 1, padding='same', name='End3')
        ...
        output = tf.squeeze(layer20, axis=[-1])
