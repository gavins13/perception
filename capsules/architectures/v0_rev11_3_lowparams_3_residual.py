...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        print(input_images.get_shape().as_list())
        layer1 = init_conv_2d(input_images, 512, "Init", kernel_size=9, type="SAME")# [batch, 1, num_ch, height, width]
        ...
        for i in range(4):
            layer2 = tf.layers.conv2d(layer2, 256, 5, padding='same', name='PrimaryCapsReduce'+str(i), activation=tf.nn.sigmoid)
        for i in range(1):
        ...
        for i in range(4):
            layer2 = tf.layers.conv2d(layer2, 256, 5, padding='same', name='1PrimaryCapsReduce'+str(i), activation=tf.nn.relu)
        for i in range(1):
        ...
        layer3 = convolutional_capsule_layer_v2(layer2,1,1,'ConvCaps1',output_kernel_vec_dim=16,  num_output_channels=1, strides=[1,1], type="SAME", num_routing=12, use_matrix_bias=False)
