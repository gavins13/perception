...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer2a = layer2
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 64, 7, padding='same', name='PrimaryCaps'+str(i), activation=tf.nn.relu)
        for i in range(4):
            layer2 = tf.layers.conv2d(layer2, 32, 5, padding='same', name='PrimaryCapsReduce'+str(i), activation=tf.nn.sigmoid)
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 128, 1, padding='same', name='PrimaryCapsReduce.2'+str(i), activation=tf.nn.sigmoid)
        layer2b = layer2
        layer2 = layer2a
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 64, 7, padding='same', name='1PrimaryCaps'+str(i), activation=tf.nn.relu)
        for i in range(4):
            layer2 = tf.layers.conv2d(layer2, 32, 5, padding='same', name='1PrimaryCapsReduce'+str(i), activation=tf.nn.relu)
        for i in range(1):
