...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer4c = layer4
        for i in range(4):
            layer4a = tf.layers.conv2d(layer4a, 64, 5, strides=(1,1), padding='same', name="capstooutput_tanh"+str(i), activation=tf.nn.tanh)
        for i in range(4):
            layer4b = tf.layers.conv2d(layer4b, 64, 5, strides=(1,1), padding='same', name="capstooutput_relu"+str(i), activation=tf.nn.relu)
        ...
        layer4 = tf.concat([layer4a, layer4b, layer4c, layer4, tf.expand_dims(input_images, axis=3)], axis=3)
        for i in range(4):
            layer4 = tf.layers.conv2d(layer4, 16, 9, strides=(1,1), padding='same', name="capstooutput_all"+str(i), activation=tf.nn.relu)
        layer4 = tf.layers.conv2d(layer4, 1, 9, strides=(1,1), padding='same', name="capstooutput_all."+str(i), activation=tf.nn.relu)
