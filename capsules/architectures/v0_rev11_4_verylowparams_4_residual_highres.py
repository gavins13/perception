...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer3 = convolutional_capsule_layer_v2(layer2,1,1,'ConvCaps1',output_kernel_vec_dim=16,  num_output_channels=1, strides=[1,1], type="SAME", num_routing=3)
        ...
        layer4 = tf.layers.conv2d(layer4, 32, 9, strides=(1,1), padding='same', name="capstooutput."+str(1), activation=tf.nn.relu)
        layer4 = tf.layers.conv2d(layer4, 16, 9, strides=(1,1), padding='same', name="capstooutput."+str(2), activation=tf.nn.relu)
        layer4 = tf.layers.conv2d(layer4, 8, 9, strides=(1,1), padding='same', name="capstooutput."+str(3), activation=tf.nn.relu)
        layer4 = tf.layers.conv2d(layer4, 1, 9, strides=(1,1), padding='same', name="capstooutput."+str(4), activation=tf.nn.relu)
        output = layer4
        #norm_layer = tf.norm(layer4, axis=3, keepdims=True)
        res_layer = tf.expand_dims(input_images, axis=3)
        output = tf.concat([output, res_layer], axis=3)
