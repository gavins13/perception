...
Results = collections.namedtuple('Results', ('output', 'capslayer'))
...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer2a = layer2
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 256, 7, padding='same', name='PrimaryCaps'+str(i), activation=tf.nn.relu)
        for i in range(4):
        ...
        layer2 = layer2a
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 256, 7, padding='same', name='1PrimaryCaps'+str(i), activation=tf.nn.relu)
        for i in range(4):
        ...
        output = tf.concat([output, norm_layer, res_layer], axis=3)
        ...
        result = Results(output, capslayer)
        print(">>>>> Graph Built!")
