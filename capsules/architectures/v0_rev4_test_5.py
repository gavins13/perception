...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        #layer2 = depthwise_convolutional_capsule_layer(layer1b, 9,9, "DepthConv2_red", strides=[2,2], num_output_channels=16, upsampling_factor=None, type="SAME", conv_vector=False)
        #layer3 = depthwise_convolutional_capsule_layer(layer2, 9,9, "DepthConv2_up", strides=[2,2], num_output_channels=16, upsampling_factor=2., type="SAME", conv_vector=False)
        ...
        layer15 =tf.layers.conv2d(layer15, 64, 9, strides=(1,1), padding='same', name="red2")
        layer15 =tf.layers.conv2d(layer15, 64, 9, strides=(1,1), padding='same', name="red3")
        layer15 =tf.layers.conv2d(layer15, 64, 9, strides=(1,1), padding='same', name="red4")
        for i in range(6,40):
            layer15 =tf.layers.conv2d(layer15, 64, 9, strides=(1,1), padding='same', name="red"+str(i))
        #layer15 = tf.nn.leaky_relu(layer15, name="relu1")
        layer15 =tf.layers.conv2d_transpose(layer15, 64, 25, strides=(2,2), padding='same', name="ups")
        #layer15 = tf.nn.leaky_relu(layer15, name="relu2")



        layer16 = tf.layers.conv2d(layer15, 64, 25, padding='same', name="End1")
        #layer17 = tf.nn.leaky_relu(layer16, name="End2")
        layer17 = layer16
        layer18 = tf.layers.conv2d(layer17, 64, 25, padding='same', name='End3')
        #layer19 = tf.nn.relu(layer18, name="End4")
        layer19=layer18
        layer20 = tf.layers.conv2d(layer19, 1, 3, padding='same', name="End5")
