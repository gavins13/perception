...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer2 = tf.layers.conv2d(layer2, 512, 5, padding='same', name='PrimaryCapsReduceInit', activation=tf.nn.relu, strides=(2,2))
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 512, 5, padding='same', name='PrimaryCapsReduce'+str(i), activation=tf.nn.relu)
        layer2 = tf.layers.conv2d(layer2, 512, 5, padding='same', name='PrimaryCapsReduceInit.2', activation=tf.nn.relu, strides=(2,2))
        for i in range(5):
            layer2 = tf.layers.conv2d(layer2, 1024, 1, padding='same', name='PrimaryCapsReduce.2'+str(i), activation=tf.nn.relu)
        # [batch, 8,8, 512]
        tmp = layer2.get_shape().as_list()
        layer2 = tf.expand_dims(layer2, axis=4)
        layer2 = tf.reshape(layer2, [tmp[0],tmp[1],tmp[2]] + [32, 32])
        ...
        if(self.method=='patch_routing'):
            ...
            with tf.variable_scope("matmul"):
                tmp = layer2.get_shape().as_list()
                matrix_shape = [1, tmp[2], tmp[3], tmp[4], 64, tmp[1]] # [1, num_ch, height, width, 16, vec_dim]
                ...
                matrix_bias = tf.tile(matrix_bias, [tmp[0], 1, tmp[3], tmp[4], 64, 1])
            ...
            layer3 = routing(layer3, 'Routing', output_dimensions=[100,1,1], squash_biases=None, num_routing=3, bias_channel_sharing=False)
        else:
        ...
        layer1b = tf.reshape(layer3, [its[0], -1])
        layer1b = tf.layers.dense(layer1b, 4096, activation=tf.nn.relu)
        layer1b = tf.layers.dense(layer1b, 4096, activation=tf.nn.relu)
        layer1b = tf.layers.dense(layer1b, 2048, activation=tf.nn.relu)
        layer1b = tf.layers.dense(layer1b, 8*8*32, activation=tf.sigmoid)
        layer1b = tf.reshape(layer1b, [its[0], 8, 8, 32])
        print("FC")
        for i in range(2):
            layer1b =tf.layers.conv2d(layer1b, 1024, 3, strides=(1,1), padding='same', name="preups_1_"+str(i), activation=tf.nn.relu)
        for i in range(1):
            layer1b =tf.layers.conv2d_transpose(layer1b, 1024, 4, strides=(2,2), padding='same', name="ups_1_"+str(i), activation=tf.nn.relu)
        for i in range(2):
            layer1b =tf.layers.conv2d(layer1b, 1024, 3, strides=(1,1), padding='same', name="preups_2_"+str(i), activation=tf.nn.relu)
        for i in range(1):
            layer1b =tf.layers.conv2d_transpose(layer1b, 1024, 4, strides=(2,2), padding='same', name="ups_3_"+str(i), activation=tf.nn.relu)
        for i in range(2):
            layer1b =tf.layers.conv2d(layer1b, 1024, 3, strides=(1,1), padding='same', name="preups_3_"+str(i), activation=tf.nn.relu)
