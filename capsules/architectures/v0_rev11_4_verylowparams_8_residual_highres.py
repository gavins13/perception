...
# 0v,11,4,  1->2 : changed the initial conv params
# 0v,11,4,  2->3 : changed the caps to output conv params
# 0v,11,4,  3->4 : take away the norm layer at the end + fewer weights at initial conv + added some more summary images + higher learning rate (1e-4 from 1.e-5)
# 0v,11,4,  4->5 : for caps output (after conv for caps only), just add output and caps output together, nothing more, to give overall output, had to change the activation at the end as a result  + learning rate change to 1e-3 from 1e-4  + DON'T USE MATRIX BIAS
# 0v,11,4,  5->6 : learning rate is lowered to 1e-5, changed the end to have linear activation (THINK-> of course tanh activation won't train!), increased num_routing = 12,
...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer2a = layer2
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 128, 7, padding='same', name='PrimaryCaps'+str(i), activation=tf.nn.relu)
        for i in range(16):
            toconv = tf.concat([layer2, tf.expand_dims(input_images, axis=3)], axis=3)
            layer2 = tf.layers.conv2d(toconv, 64, 5, padding='same', name='PrimaryCapsReduce'+str(i), activation=tf.nn.relu)
        for i in range(1):
            toconv = tf.concat([layer2, tf.expand_dims(input_images, axis=3)], axis=3)
            layer2 = tf.layers.conv2d(toconv, 256, 5, padding='same', name='PrimaryCapsReduce.2'+str(i), activation=None)
        ...
        layer3 = convolutional_capsule_layer_v2(layer2,1,1,'ConvCaps1',output_kernel_vec_dim=16,  num_output_channels=1, strides=[1,1], type="SAME", num_routing=3, use_matrix_bias=False, use_squash_bias=False)
        #layer3 = convolutional_capsule_layer_v2(layer3,1,1,'ConvCaps2',output_kernel_vec_dim=32,  num_output_channels=1, strides=[1,1], type="SAME", num_routing=3, use_matrix_bias=False)
        ...
        for i in range(10):
            layer4 =tf.layers.conv2d(layer4, 32, 9, strides=(1,1), padding='same', name="capstooutput"+str(i), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 8, 9, strides=(1,1), padding='same', name="capstooutput."+str(1), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 8, 9, strides=(1,1), padding='same', name="capstooutput."+str(2), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 8, 9, strides=(1,1), padding='same', name="capstooutput."+str(3), activation=None)
        layer4 = tf.layers.conv2d(layer4, 1, 9, strides=(1,1), padding='same', name="capstooutput."+str(4), activation=None)
        output = layer4
        ...
        with tf.device('/cpu:0'):
            with tf.name_scope('output_addition'):
                weight_1 = tf.get_variable('weight1',[1],initializer=tf.constant_initializer(0.1), dtype=tf.float32, trainable=True)
                weight_2 = tf.get_variable('weight2',[1],initializer=tf.constant_initializer(2.0), dtype=tf.float32, trainable=True)
        ...
        #output = tf.layers.conv2d(output, 1, 1, strides=(1,1), padding='same', name="thefinaloutput."+str(4), use_bias=False)
        #output = tf.squeeze(output, axis=3)


        #output = tf.add(output, input_images)
        print(output.get_shape().as_list())
        capslayer = tf.transpose(tf.squeeze(layer3, axis=2), [1,2,3,0])
