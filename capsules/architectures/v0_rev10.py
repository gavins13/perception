...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        print(input_images.get_shape().as_list())
        layer1 = init_conv_2d(input_images, 2048, "Init", kernel_size=13, type="SAME")# [batch, 1, num_ch, height, width]
        ...
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 2048, 5, padding='same', name='PrimaryCaps'+str(i), activation=tf.nn.relu)
        for i in range(2):
            layer2 = tf.layers.conv2d(layer2, 2048, 5, padding='same', name='PrimaryCapsReduce'+str(i), activation=tf.nn.relu)
        for i in range(3):
            layer2 = tf.layers.conv2d(layer2, 2048, 1, padding='same', name='PrimaryCapsReduce.2'+str(i), activation=tf.nn.relu)
        # [batch, 8,8, 512]
        tmp = layer2.get_shape().as_list()
        layer2 = tf.expand_dims(layer2, axis=4)
        layer2 = tf.reshape(layer2, [tmp[0],tmp[1],tmp[2]] + [32, 64])
