...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer1 = tf.layers.conv2d(layer1, 128, 9, padding='same', name='InitConv', activation=tf.nn.relu)
        layer2 = layer1
        layer2 = tf.layers.conv2d(layer2, 128, 9, padding='same', name='linear_conv', activation=None)
        # [batch, 128,128, 256]
        tmp = layer2.get_shape().as_list()
        layer2 = tf.expand_dims(layer2, axis=4)
        layer2 = tf.reshape(layer2, [tmp[0],tmp[1],tmp[2]] + [8, 16])
        ...
        layer4 = tf.layers.conv2d(layer4, 128, 9, strides=(1,1), padding='same', name="capstooutput"+str(1), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 128, 9, strides=(1,1), padding='same', name="capstooutput."+str(1), activation=None)
