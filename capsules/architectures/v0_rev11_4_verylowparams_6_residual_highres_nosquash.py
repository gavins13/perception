...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        for i in range(4):
            layer2 = tf.layers.conv2d(layer2, 128, 5, padding='same', name='PrimaryCapsReduce'+str(i), activation=tf.nn.sigmoid)
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 128, 1, padding='same', name='PrimaryCapsReduce.2'+str(i), activation=tf.nn.tanh)
        layer2b = layer2
        layer2 = layer2a
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 128, 7, padding='same', name='1PrimaryCaps'+str(i), activation=tf.nn.relu)
        for i in range(4):
            layer2 = tf.layers.conv2d(layer2, 128, 5, padding='same', name='1PrimaryCapsReduce'+str(i), activation=tf.nn.relu)
        for i in range(1):
            layer2 = tf.layers.conv2d(layer2, 128, 1, padding='same', name='1PrimaryCapsReduce.2'+str(i), activation=tf.nn.tanh)
        layer2a = layer2
        ...
        layer3 = convolutional_capsule_layer_v2(layer2,1,1,'ConvCaps1',output_kernel_vec_dim=16,  num_output_channels=1, strides=[1,1], type="SAME", num_routing=4, use_matrix_bias=False, use_squash_bias=False)
        ...
        for i in range(1):
            layer4 =tf.layers.conv2d(layer4, 64, 9, strides=(1,1), padding='same', name="capstooutput"+str(i), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 32, 9, strides=(1,1), padding='same', name="capstooutput."+str(1), activation=tf.nn.tanh)
        layer4 = tf.layers.conv2d(layer4, 32, 9, strides=(1,1), padding='same', name="capstooutput."+str(2), activation=tf.nn.tanh)
        ...
        output = tf.layers.conv2d(output, 1, 1, strides=(1,1), padding='same', name="thefinaloutput."+str(4), activation=None, use_bias=False)
        output = tf.squeeze(output, axis=3)
