...
# This architecture is just about produced a CNN-like capsule network!


# NOTE: this proposed architecture was not due to favourability to run the more iterative SENSE-like process in v1_rev3_1
...
class architecture(architecture_base):
    def build(self, input_images):
        ...
        for i in range(0):
            layer1 = tf.layers.conv2d(layer1, 64, 9, padding='same', name='InitConv'+str(i), activation=tf.nn.relu)
        layer2 = layer1
        ...
        for caps_layer_num in range(5):
            with tf.name_scope("caps_layer_" + str(caps_layer_num)):
                with tf.variable_scope("caps_layer_" + str(caps_layer_num)):
                    ...
                    layer4c = layer4
                    for i in range(1):
                        layer4a = tf.layers.conv2d(layer4a, 16, 9, strides=(1,1), padding='same', name="capstooutput_tanh"+str(i), activation=tf.nn.tanh)
                    for i in range(1):
                        layer4b = tf.layers.conv2d(layer4b, 16, 9, strides=(1,1), padding='same', name="capstooutput_relu"+str(i), activation=tf.nn.relu)
                    for i in range(1): # deep but provides ability to pull the background characteristics together
                        layer4c = tf.layers.conv2d(layer4c, 4, 9, strides=(1,1), padding='same', name="capstooutput_linear"+str(i), activation=None)
                    layer4 = tf.concat([layer4a, layer4b, layer4c, layer4], axis=3)
                    layer4 = tf.layers.conv2d(layer4, 16, 9, strides=(1,1), padding='same', name="capstooutput_all"+str(4), activation=None)
                    layer2 = tf.expand_dims(layer4, axis=4)
                    layer2 = tf.reshape(layer2, [tmp[0],tmp[1],tmp[2]] + [4, 4])
                    layer2 = tf.transpose(layer2, [0,3,4,1,2])



        layer4 = tf.layers.conv2d(layer4, 1, 9, strides=(1,1), padding='same', name="capstooutput_all."+str(4), activation=None)
        output = tf.squeeze(layer4, axis=3)
