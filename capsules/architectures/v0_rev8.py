...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        for i in range(5):
            layer2 = tf.layers.conv2d(layer2, 512, 1, padding='same', name='PrimaryCapsReduce.2'+str(i), activation=tf.nn.relu)
        # [batch, 8,8, 512]
        tmp = layer2.get_shape().as_list()
        layer2 = tf.expand_dims(layer2, axis=4)
        layer2 = tf.reshape(layer2, [tmp[0],tmp[1],tmp[2]] + [8, 64])
        ...
        if(self.method=='patch_routing'):
            ...
            layer3 = routing(layer3, 'Routing', output_dimensions=[200,1,1], squash_biases=None, num_routing=3, bias_channel_sharing=False)
        else:
