...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer15 =tf.layers.conv2d(layer15, 128, 25, strides=(2,2), padding='same', name="red")
        layer15a = layer15
        layer15 = tf.nn.relu(layer15)
        for i in range(6,8):
            layer15 =tf.layers.conv2d(layer15, 64, 9, strides=(1,1), padding='same', name="red"+str(i))
            layer15 = tf.nn.relu(layer15)
        layer15 =tf.layers.conv2d_transpose(layer15, 128, 24, strides=(2,2), padding='same', name="ups")
        layer15 = tf.nn.relu(layer15)
        ...
        layer15a = tf.nn.relu(tf.layers.conv2d(layer15a, 128, 9, strides=(1,1), padding='same', name="conv1"))
        layer15a = tf.nn.relu(tf.layers.conv2d(layer15a, 128, 9, strides=(1,1), padding='same', name="conv2"))
        layer15 = tf.concat([layer15, layer15a], axis=3)



        layer16 = tf.layers.conv2d(layer15, 128, 3, padding='same', name="End1")
