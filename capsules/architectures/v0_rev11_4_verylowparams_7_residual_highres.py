...
class architecture(object):
    def __init__(self):
        self.hparams = tf.contrib.training.HParams(
          decay_rate=0.9,
          decay_steps=1000.,
          learning_rate=1.e-9, # 0.001
          loss_type='margin',
          padding='VALID',
          remake=True,
          verbose=False,
          maximum_learning_rate = 1.e-12, # 1.e-7
        )
    ...
    def build(self, input_images):
        ...
        for i in range(32):
            ...
            layer2 = tf.layers.conv2d(toconv, 32, 5, padding='same', name='PrimaryCapsReduce'+str(i), activation=tf.nn.relu)
        for i in range(1):
            toconv = tf.concat([layer2, tf.expand_dims(input_images, axis=3)], axis=3)
            layer2 = tf.layers.conv2d(toconv, 256, 5, padding='same', name='PrimaryCapsReduce.2'+str(i), activation=tf.nn.relu)
        ...
        for i in range(10):
            layer4 =tf.layers.conv2d(layer4, 16, 9, strides=(1,1), padding='same', name="capstooutput"+str(i), activation=tf.nn.tanh)
