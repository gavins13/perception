...
# 0v,13, 2->3 => Init conv layers incresaed from 3 to 6. Also, change from 3 routing iterations to 1 routing iteration. (It makes sense that more iterations should only be used for more classification based tasks?)
...
class architecture(object):
    ...
    def build(self, input_images):
        ...
        layer3 = convolutional_capsule_layer_v2(layer2,1,1,'ConvCaps1',output_kernel_vec_dim=4,  num_output_channels=4, strides=[1,1], type="SAME", num_routing=1, use_matrix_bias=True, use_squash_bias=True)
        ...
        layer4a = tf.layers.conv2d(layer4, 255, 9, strides=(1,1), padding='same', name="capstooutput"+str(1), activation=tf.nn.tanh)
        layer4 = tf.concat([layer4a, layer4], axis=3)
