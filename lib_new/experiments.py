...
exp_ext = '.json'
json_files = [f for f in listdir('experiments/')\
    if isfile(join(mypath, f)) and f.find(exp_ext)==len(exp_ext)-5]
...
class Experiments(object):
    ...
    def save(self, experiment_id, folder_name):
        file_location = self.json_locations[experiment_id]
        self.experiments[experiment_id]["save_folder"] = folder_name
        self.experiments_json_files[file_location][experiment_id]["save_folder"] = folder_name
        self.update_json_file()
    def update_json_file(self, json_file):
        dict = self.experiments_json_files[json_file]
    ...
    def reset(self, experiment_id):
        ...
        '''
        file_location = self.json_locations[experiment_id]
        self.experiments[experiment_id]["save_folder"] = None
        self.experiments_json_files[file_location][experiment_id]["save_folder"] = None
        self.update_json_file()
