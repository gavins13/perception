...
class architecture(architecture_base):
    ...
    def build(self, input_images, k_space_mask, k_space_masked, k_space_full, add_noise=False):
        ...
        input_images_real = tf.expand_dims(tf.real(input_images), axis=3)
        input_images_imag = tf.expand_dims(tf.imag(input_images), axis=3)
        ...
        def cnn(input_images, nd=5):
            ...
            for i in range(nd-1-vec_to_output_layers):
                layer1 = tf.layers.conv2d(layer1, 64, 3, padding='same', name='Conv'+str(i), activation=tf.nn.relu, kernel_initializer=tf.keras.initializers.he_normal(seed=None))
            layer1 = tf.layers.conv2d(layer1, 32, 3, padding='same', name='ConvCon1'+str(i), activation=tf.nn.relu, kernel_initializer=tf.keras.initializers.he_normal(seed=None))
            layer1 = tf.layers.conv2d(layer1, 16, 3, padding='same', name='ConvCon2'+str(i), activation=tf.nn.relu, kernel_initializer=tf.keras.initializers.he_normal(seed=None))
            def onehot_capsule(layer1):
              output_channels = 4
              normalise = True
              layer1 = tf.transpose(tf.reshape(tf.expand_dims(layer1, axis=4), its[0:3] + [4,4]), [0,3,4,1,2])
              ...
              layer4 = tf.norm(layer1, ord='euclidean', axis=1,  keepdims=False)   # [M, |T^l+1|, x,y]
              layer4_ind = tf.argmax(tf.transpose(layer4, [0,2,3,1]), axis=3) # [M, x,y]
              layer4_mask = tf.one_hot(layer4_ind, depth=output_channels, on_value=1., off_value=0., axis=None) # [M,x,y,T^l+1]
              print("//EndOneHot")
              capschannelvotes = tf.transpose(layer4_mask, [1,2,3,0])
              layer4_mask = tf.transpose(tf.expand_dims(layer4_mask, axis=4), [0,4,3,1,2]) # [M, 1, T^l+1, x , y]
              ...
              layer6 = tf.squeeze(tf.reshape(tf.transpose(layer5, [0,3,4,1,2]), its[0:3]+[-1,1]), axis=4) #  [M,, x, y, v_d^l+1*|T^l+1|]
              return layer6
            layer1 = onehot_capsule(layer1)
            print("//end one hot capsule")
            for i in range(vec_to_output_layers):
                layer1 = tf.layers.conv2d(layer1, 64, 3, padding='same', name='ConvEnd'+str(i), activation=tf.nn.relu, kernel_initializer=tf.keras.initializers.he_normal(seed=None))
            layer2 = tf.layers.conv2d(layer1, 2, 3, padding='same', name='Conv_Recon', activation=tf.nn.relu, kernel_initializer=tf.keras.initializers.he_normal(seed=None))
            return layer2
        ...
        def data_consistency(input_images, l):
            ...
            def weighted_mean(raw, predict):
                ...
                ll_3 = tf.reciprocal(ll_2, name="recip1")
            ...
            k_space = tf.spectral.fft2d(input_images)
            print(">>K-space shape")
            ...
            _tmp_image_space_DC = tf.abs(tf.spectral.ifft2d(k_space_DC))
            print(">> DC")
            new_k_space = tf.add(new_k_space, k_space_DC)
            new_input_images = tf.spectral.ifft2d(new_k_space)
            new_input_images_real = tf.expand_dims(tf.real(new_input_images), axis=3)
            new_input_images_imag = tf.expand_dims(tf.imag(new_input_images), axis=3)
        ...
        _tmp_from_k_gt = tf.abs(tf.spectral.ifft2d(tf.cast(k_space_full, dtype=tf.complex64)))
    ...
    def loss_func(self, input_images, ground_truth, validation_input_images, validation_ground_truth, extra_data, validation_extra_data):
        ...
        def find_norm_new(complex_input):
            real_part = tf.real(complex_input)
            imag_part = tf.imag(complex_input)
            real = tf.pow(real_part, 2.)
            imag = tf.pow(imag_part, 2.)
            end = tf.add(real, imag)
            final = tf.reduce_sum(end, axis=[1,2], keepdims=False)
            return final
        def find_norm(input):
            #return tf.norm(input, axis=[1,2])
            input = tf.pow(tf.abs(input), 2)
            return tf.reduce_sum(input, axis=[1,2])
        ...
        with tf.name_scope('accuracy'):
            ...
            tf.summary.image('diagnostic_1', tf.expand_dims(tf.slice(res.diagnostic_1, [0,0,0], [1, -1,-1]), axis=3))
            tf.summary.image('diagnostic_2', tf.expand_dims(tf.slice(res.diagnostic_2, [0,0,0], [1, -1,-1]), axis=3))



        print(">>>> Add results to output (validation)")
        with tf.name_scope('validation'):
            tf.summary.image('input', tf.expand_dims(tf.abs(validation_input_images), axis=3))
            tf.summary.image('ground_truth', tf.expand_dims(validation_ground_truth, axis=3))
            tf.summary.image('output', tf.expand_dims(tf.abs(validation_output), axis=3))
            tf.summary.scalar('psnr', tf.reduce_mean( tf.image.psnr(tf.expand_dims(validation_ground_truth, axis=3), tf.abs(tf.expand_dims(validation_output, axis=3)), max_val=1114)))
            tf.summary.scalar('ssim', tf.reduce_mean( tf.image.ssim(tf.expand_dims(validation_ground_truth, axis=3), tf.abs(tf.expand_dims(validation_output, axis=3)), max_val=1114)))
            tf.summary.image('k_space_real_gt', tf.expand_dims(tf.slice(tf.real(validation_k_space_masked), [0,0,0], [1, -1,-1]), axis=3))
            tf.summary.image('k_space_image_gt', tf.expand_dims(tf.slice(tf.imag(validation_k_space_masked), [0,0,0], [1, -1,-1]), axis=3))
            tf.summary.image('k_space_real', tf.expand_dims(tf.slice(tf.real(validation_res.k_space_pred), [0,0,0], [1, -1,-1]), axis=3))
            tf.summary.image('k_space_image', tf.expand_dims(tf.slice(tf.imag(validation_res.k_space_pred), [0,0,0], [1, -1,-1]), axis=3))
            tf.summary.image('k_space_mask', tf.expand_dims(tf.slice(tf.real(validation_k_space_mask), [0,0,0], [1, -1,-1]), axis=3))
            tf.summary.image('diagnostic_1', tf.expand_dims(tf.slice(validation_res.diagnostic_1, [0,0,0], [1, -1,-1]), axis=3))
            tf.summary.image('diagnostic_2', tf.expand_dims(tf.slice(validation_res.diagnostic_2, [0,0,0], [1, -1,-1]), axis=3))
